...
export const AdminManagerDashboard: React.FC = memo(() => {
  ...
  const overviewContent = (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <GlassStat title="Revenu" value={`${(stats?.totalRevenue ?? 0).toLocaleString()} DA`} icon={<TrendingUp className="h-5 w-5" />} color="amber" />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <GlassStat title="Utilisateurs" value={stats?.totalUsers ?? 0} icon={<Users className="h-5 w-5" />} color="primary" />
        <GlassStat title="Managers" value={stats?.activeManagers ?? 0} icon={<Shield className="h-5 w-5" />} color="secondary" />
        <GlassStat title="Groupes" value={stats?.totalGroups ?? 0} icon={<FolderKanban className="h-5 w-5" />} color="accent" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">Score moyen de confiance</p>
          </CardContent>
        </Card>
        ...
        <GlassCard className="p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between"><span className="text-sm">En attente</span><Badge variant="outline" className="text-amber-600 border-amber-600">{stats?.pendingVerifications ?? 0}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-sm">Approuvées</span><Badge className="bg-emerald-600 hover:bg-emerald-700">{stats?.approvedVerifications ?? 0}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-sm">Rejetées</span><Badge variant="destructive">{stats?.rejectedVerifications ?? 0}</Badge></div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg">Mes propriétés récentes</CardTitle>
              <Button variant="ghost" size="sm" onClick={() => navigate('/properties')} className="gap-1">Voir tout <ArrowRight className="h-4 w-4" /></Button>
            </div>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle className="text-lg">Réservations récentes</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader><CardTitle className="text-lg">Demandes entrantes</CardTitle></CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <GlassCard className="p-6">
          <h3 className="text-lg font-semibold flex items-center gap-2 mb-4"><Shield className="h-5 w-5 text-primary" /> Équipe</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between"><span className="text-sm">Admins</span><Badge>{stats?.totalAdmins ?? 0}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-sm">Managers</span><Badge variant="secondary">{stats?.totalManagers ?? 0}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-sm">Utilisateurs</span><Badge variant="outline">{stats?.totalRegularUsers ?? 0}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-sm">Assignations</span><Badge variant="outline">{stats?.totalAssignments ?? 0}</Badge></div>
          </div>
        </GlassCard>

        <GlassCard variant="accent" className="p-6">
          <h3 className="text-lg font-bold flex items-center gap-2 mb-4"><Zap className="h-5 w-5 text-primary" /> Actions rapides</h3>
          <div className="flex flex-wrap gap-2">
            {isAdmin && <Button onClick={() => setInviteOpen(true)} className="gap-2"><UserPlus className="h-4 w-4" /> Inviter Manager</Button>}
            <Button variant="outline" onClick={() => navigate('/properties/new')} className="gap-2"><PlusCircle className="h-4 w-4" /> Propriété</Button>
            <Button variant="outline" onClick={() => navigate('/services/new')} className="gap-2"><Compass className="h-4 w-4" /> Service</Button>
            <Button variant="outline" onClick={() => navigate('/properties')} className="gap-2"><Building2 className="h-4 w-4" /> Mes Propriétés</Button>
          </div>
        </GlassCard>
      </div>
      ...
      {assignments.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" /> Assignations</h3>
          {assignments.map(assignment => {
            const perms = permissionsMap[assignment.id] || [];
            const granted = perms.filter(p => p.isGranted);
            const scopeLabel = assignment.scope === 'all' ? 'Toutes les propriétés' : assignment.scope === 'property' ? assignment.property?.title || 'Propriété' : assignment.propertyGroup?.name || 'Groupe';
            ...
            return (
              <GlassCard key={assignment.id} variant="elevated" className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <Badge variant={assignment.isActive ? 'default' : 'outline'}>{assignment.isActive ? 'Actif' : 'Inactif'}</Badge>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border/40">
                  {hasPermission(assignment.id, 'view_bookings') && (
                    <Button variant="outline" size="sm" onClick={() => navigate('/bookings/host')} className="gap-1.5"><CalendarCheck className="h-3.5 w-3.5" /> Réservations</Button>
                  )}
                  ...
                  {hasPermission(assignment.id, 'modify_prices') && (
                    <Button variant="outline" size="sm" onClick={() => navigate('/properties')} className="gap-1.5"><DollarSign className="h-3.5 w-3.5" /> Tarifs</Button>
                  )}
                </div>
              </GlassCard>
            );
  ...
  const tabs = [
    ...
    { value: 'points', label: 'Points & Récompenses', icon: <Trophy className="h-4 w-4" />, content: (
      <ErrorBoundary>
        <div className="space-y-6">
    ...
    { value: 'fees', label: 'Frais & Marges', icon: <DollarSign className="h-4 w-4" />, content: (
      <ErrorBoundary>
        <div className="space-y-6">
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdmin && (
        <InvitationForm open={inviteOpen} onOpenChange={setInviteOpen} allowedRoles={['manager']} onSuccess={loadData} />
      )}
    </div>
  );
});
