...
export class ServiceFeeController {
  ...
  @Post('calculate')
  @RequireRole('hyper_admin', 'hyper_manager', 'admin')
  async calculate(@Body() body: { hostId: number; propertyId: string; propertyGroupId?: string; amount: number }) {
    return this.service.calculateFee(body.hostId, body.propertyId, body.propertyGroupId || null, body.amount);
  }
}
